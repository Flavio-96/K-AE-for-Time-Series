...
Il numero di intervalli di tempo definiscono la cosiddetta \textbf{lunghezza} della time series, e quindi la sua dimensionalità.
\\
...
Per affrontare il problema descritto si farà uso di un \textbf{autoencoder}, una particolare rete neurale in grado di fornire una rappresentazioni compatta dei suoi input (qui le TS), detta \textbf{codifica} o \textbf{vettore latente}, di dimensione ridotta e decisa tramite gli iperparametri del modello.\\
...
Per ogni esecuzione di k-Means vengono mostrati i risultati tramite misure di validazione del clustering.\\
\\
I risultati del clustering con l'autoencoder saranno poi confrontati con il k-Means attuato dalla libreria \textbf{TSLearn} e con i risultati di altre tecniche di feature extraction and selection basate su \textbf{TSFresh}.\\
L'analisi compatariva ha il solo di scopo di valutare le prestazioni sui soli dataset in esame e su determinate categorie di TS, non per poter decretare la tecnica migliore in assoluto.\\
\\
